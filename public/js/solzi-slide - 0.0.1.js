var Slide = function (arg) {
	...
	this.$slides = []; // 생성될 .solzi-slide(jQuery객체)를 저장할 배열
...
Slide.prototype.init = function () {
	...
	// stage에 슬라이드를 감싸는 solzi-wrapper를 생성
	this.$wrapper = $('<div class="solzi-wrapper solzi-' + this.direction + '"></div>').appendTo(this.$container);

	// solzi-wrapper에 solzi-slide들을 만드는 과정
	for (var i = 0, html = ''; i < this.slide.length; i++) {
		html = '<div class="solzi-slide">';
		html = '<div class="solzi-slide" style="background-image: url(\'' + this.slide[i] + '\');">';
		html += '<img src="' + this.slide[i] + '" class="w100">';
		if (this.title && this.title[i]) html += this.title[i];
		html += '</div>';

		// hori, vert 형은 solzi-wrapper에 실제 태그를 넣어놔야 되므로... 
		// 1. 실제태그도 붙이고 
	...
	// fade type은 0번 슬라드를 solzi-wrapper에 붙인다.
	...
	if (this.btnUse) {
		this.$btnPrev = $('<div class="solzi-btn solzi-prev">〈</div>').appendTo(this.$container);
		this.$btnNext = $('<div class="solzi-btn solzi-next">〉</div>').appendTo(this.$container);
	...
	if (this.pagerUse) {
		...
		// solzi-pagers를 this.$container에 생성한다.
		this.$pagers = $('<div class="solzi-pagers"></div>').appendTo(this.$container);

		// 각각의 pager를 생성하고 클릭 이벤트를 적용한다.
		for (var i in this.slide) {
			$('<div class="solzi-pager"></div>').appendTo(this.$pagers).click(this.onPagerClick.bind(this));
		}

		// 0번 idx에 .active 클래스를 적용한다.
		this.$pagers.find(".solzi-pager").eq(0).addClass("active");
	}
	...
	if (this.direction == 'step') {
		$(window).resize(function () {
			...
			this.$wrapper.find('.solzi-slide').css({
...
// solzi-wrapper의 animation
Slide.prototype.ani = function () {
	if (this.pagerUse) {
		this.$pagers.find(".solzi-pager").eq(this.now).addClass("active").siblings().removeClass("active");
		if (this.direction === 'hori' || this.direction === 'vert') {
			if (this.now == this.last) this.$pagers.find(".solzi-pager").eq(0).addClass("active").siblings().removeClass("active");
		}
	}
